# 布局对齐：MediaGeneratorWorkspace
- 首页 `HeroSection` 使用 `mx-auto max-w-7xl px-6` 宽度容器；为了让 `MediaGeneratorWorkspace` 视觉宽度与 header 一致，需要在首页使用同样的容器包裹组件，再让内部继续 `w-full` 以占满容器。
- 在独立的 Media Generator 页面中，仍然把 `MediaGeneratorWorkspace` 直接放在 `w-full` 的 `main` 内，保证它占满全屏，方便展示全宽体验。

# Media Generator 命名与默认模型
- 营销页路径与组件从 `video-generator` 更名为 `media-generator`，对应 Workspace/Section 组件也统一 Media 前缀，并将营销布局的 footer 隐藏规则改为匹配 `/media-generator`。
- 模型配置抽到 `soraVideoPreset`（包含 mediaType/modelName/size/seconds），Workspace 默认引用该 preset，Editor Panel 顶部按钮实时展示当前模型名，后续新增模型时只需补充 preset 并透传。
...
- 前端在 `MediaGeneratorPreviewPanel` 中使用 `useCurrentUser` 判断是否登录，已登录时触发 `loadFeed({ reset: true })`，并用 `nextCursorRef`/`hasMoreRef` 管理游标翻页。
...
# Media Generation Trigger & Polling
...
- `MediaGeneratorWorkspace` 现在驱动真实生成：`handleGenerate` POST `/api/media/generate`，获得 `taskId` 后将状态保存到 `activeGeneration`，并以 `setInterval` 每 5s 调用 `/api/media/result/{taskId}` 更新 `status / progress / video URL`，终态（completed/failed/timeout）会自动停止轮询。
...
# Media Generator 子页与组件化设计要点
- 拆出 `MediaGeneratorShell`（含菜单）与 `MediaGeneratorEmbed`（隐藏菜单），子页直接复用后者传入默认媒体类型、默认模型和占位媒资。
- 使用 `MediaGeneratorProvider` 统一管理媒体类型、模型、参数与生成结果，`MediaGeneratorWorkspace` 只消费 Context，降低 props 传递。
- 通用生成按钮封装为 `GenerateAction`，负责构建请求、触发 `/api/media/generate` + 轮询 `/api/media/result/{taskId}`，并写回状态供预览区更新。
- 模型配置按模型拆分（如 `SoraConfigPanel`、`Veo3ConfigPanel`），通过映射函数将 UI 字段转成接口体，方便扩展新模型或媒体类型。
- 预览区 `GenerationPreview` 接收 `defaultAsset`，未生成时展示占位，生成后仅显示最新结果，不保留历史列表以简化子页体验。
