...
- **配置表单拆分**：按媒体类型放置配置组件：`video/sora-config-fields.tsx`、`video/veo3-config-fields.tsx` 等模型独立文件，`image/config-fields.tsx`、`audio/config-fields.tsx` 负责对应媒体；共享控件在 `shared/config-field-controls.tsx`，聚合导出集中于 `model-configs.tsx`，便于增删模型时只改所属文件与注册表。
