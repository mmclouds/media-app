...
- **"use client"**：让组件可以使用 `useEffect` 轮询 `/api/gateway/media/result/{taskId}` 和 DOM 事件（滚动、视频 hover）。
- **App Router**：`media-studio/page.tsx` 现在引入三栏版 `MediaGeneratorWorkspace`。
- **数据策略**：两个 Workspace 通过客户端 fetch `/api/gateway/media/generate`、`/api/gateway/media/result`、`/api/gateway/media/feed`。
- **结构影响**：交互组件位于 `components/marketing/media-generator`，可按需选择三栏/双栏变体。

### 🟦 C. 代码逻辑拆解与架构说明
- **骨架**：`media-generator-workspace.tsx`（三栏）与 `media-only-generator-workspace.tsx`（双栏）都组合 `MediaGeneratorConfigPanel` + `MediaGeneratorResultPane`，并从 `useMediaGeneratorController` 获取数据流。
...
- **配置表单拆分**：按媒体类型放置配置组件：`video/sora-config-fields.tsx`、`video/veo3-config-fields.tsx` 等模型独立文件，`image/still-image-config-fields.tsx`、`audio/audio-craft-config-fields.tsx` 负责对应媒体；共享控件在 `shared/config-field-controls.tsx`，聚合导出集中于 `model-configs.tsx`，便于增删模型时只改所属文件与注册表。
