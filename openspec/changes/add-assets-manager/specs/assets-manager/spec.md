...
### 需求：核心组件可复用
系统必须将核心展示逻辑封装为独立组件，支持弹窗和页面两种使用方式。

#### 场景：弹窗模式使用
- **当** 使用 `AssetsManagerDialog` 组件
- **那么** 资产管理功能以弹窗形式展示
- **并且** 显示关闭按钮

#### 场景：页面模式使用（后续扩展）
- **当** 使用 `AssetsManager` 组件并设置 `mode="page"`
- **那么** 资产管理功能可直接嵌入页面
- **并且** 不显示关闭按钮
...
```
GET /api/media/feed
  ?mediaTypes=VIDEO|IMAGE|AUDIO  # 媒体类型筛选
  &limit=20                       # 每页数量（默认10，最大100）
  &cursor=<base64>                # 游标分页
  &sort=desc                      # 排序方向

响应格式:
{
  "content": MediaFeedItem[],     # 资产列表
  "hasMore": boolean,             # 是否有更多数据
  "nextCursor": string | null     # 下一页游标
}
```

> **说明**：此接口已存在，本功能直接复用，无需后端开发。
